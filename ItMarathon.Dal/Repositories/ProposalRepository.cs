...
{
    public async Task<IEnumerable<Proposal>> GetProposalsAsync(bool trackChanges, ODataQueryOptions queryOptions)//add from Oleksandr
    {
        IQueryable<Proposal> query = FindAll(trackChanges);

        if (queryOptions != null)
        {
            query = (IQueryable<Proposal>)queryOptions.ApplyTo(query);//add from Oleksandr
        }
