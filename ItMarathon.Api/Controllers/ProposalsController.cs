...
public class ProposalsController(IProposalService proposalService, IMapper mapper) : ControllerBase
{
    ...
    public async Task<ActionResult<DataPage<ProposalDto>>> GetAllProposals(
        [FromQuery(Name = "$top")] int? top,//add from Oleksandr
        [FromQuery(Name = "$skip")] int? skip,//add from Oleksandr
        [FromQuery(Name = "$filter")] string? filter,//add from Oleksandr
        [FromQuery(Name = "$orderby")] string? orderby)//add from Oleksandr
    {
